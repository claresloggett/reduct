...
@app.callback(
    Output('pc_composition','children'),
    [Input('x_dropdown','value'), Input('y_dropdown','value')],
    state=[State('hidden_data_div', 'children')]
)
def update_pc_composition(x_field, y_field, stored_data):
    print("Updating PC composition text")
    if stored_data=="":
    ...
    table_rows_x = [html.Tr([html.Td(field),html.Td(sqvalue)])
                      for (field, sqvalue)
                      in pcx_original.sort_values(ascending=False)[:5].items()
                      if sqvalue > 0.01]
    table_rows_y = [html.Tr([html.Td(field),html.Td(sqvalue)])
                      for (field, sqvalue)
